...
void Node::frustumCull(Camera *cam) {
	unsigned int zero=0;
	int emaitza = cam->checkFrustum(m_containerWC,&zero);
	if(emaitza==1){setCulled(true);}
	else{
		if(emaitza==-1){
			setCulled(false);
		}
...
const Node *Node::checkCollision(const BSphere *bsph) const {
	...
	/* =================== PUT YOUR CODE HERE ====================== */
	if (m_gObject != 0) { // Objektua du
		if (BSphereBBoxIntersect(bsph, m_containerWC) == IINTERSECT) return this;
		else return 0;
	}
	else { // Umeak ditu
		if (BSphereBBoxIntersect(bsph, m_containerWC) == IINTERSECT) {
			const Node * umearenEmaitza;
			for(list<Node *>::const_iterator it = m_children.begin(), end = m_children.end();
        		it != end; ++it) {
        		const Node *theChild = *it;
        		umearenEmaitza = theChild->checkCollision(bsph);
        		if (umearenEmaitza != 0) return umearenEmaitza;
    		}
